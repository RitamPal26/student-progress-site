...
const API_BASE =
  process.env.NODE_ENV === "production"
    ? "https://student-progress-site.onrender.com/api"
...
export const dataAPI = {
  getContests: (studentId, range = 90) =>
    axios.get(
      `${API_BASE}/data/contests?studentId=${studentId}&range=${range}`
    ),
  getSubmissions: (studentId, after) =>
    axios.get(
      `${API_BASE}/data/submissions?studentId=${studentId}&after=${after}`
    ),
};
